...
const protectAux = async (token, next) => {
  if (!token) {
    throw new AppError('Your are not logged in! Please log in to get access.', 401);
  }
  // verification
  ...
  if (freshUser.active === false) return next(new AppError('Please activate your account to continue.', 401));
